...
- **Next two steps**:
  ...
  2. 🔄 **A100 queue running** (single-seed 42; win = Δ ≥ G=0.025 over μ₀=0.5683 + no precision drop): `phase3_loss_compound_2to1_seed43` (resumed from e20 after the 2026-06-10 VM interruption; as of 17:00 UTC at epoch ~60, curriculum 1:15, best smoothed **0.576** and still improving — vs seed42's 0.6035) → `phase3_boundary_ignore_w1/w2/w3` (§6.2) → `phase3_wd_5e2` (§6.3) → `phase3_aug_strong` (queue armed, waiting on seed43). Then: data-scaling slope (§5.3) + Phase-5 gate eval (§8.1). Gated Phase-5 (`phase5_*`) kept but not queued. H100 spin-retry still cycling on stockout (attempt ~64). Note: seed43's current `run_summary.md` is a stale artifact of the crashed first resume attempt — it is overwritten when the run completes.
