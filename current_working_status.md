...
- **2026-06-15** — **Phase 3.1 (loss family) done → focal stays winner; Phase 3.2+ wave launched on `phase3-ablations`.** Final `best_smoothed`: compound_1to2 **0.7998 (+0.0086)**, compound_2to1 0.7933, compound_1to1 0.7878, tversky_3to7 0.590, tversky_2to8 0.073. **No loss beats the focal baseline by G=0.0112** → by §1.4, **focal is the loss winner**; compound_1to2 is a near-miss (within 1σ of the gate) so it's carried into the boundary factorial + gets a seed-43 confirm. Launched an 11-run wave via `run_gpu_pool.sh` (8/8 GPUs): **§6.2 boundary factorial** {focal, compound_1to2}×ignore{w1,w2,w3} (6) + seed-43 confirm (1) + **§8 architecture sweep** smp decoders DeepLabV3+/FPN/PSPNet/MAnet (4, as the independent backfill). New code on `phase3-ablations`: `_SMP_DECODERS` in `models/segmentation.py`; configs via one-level base-inheritance; `scripts/{stage_local,merge_mlflow_stores,annotate_overlay_labels}.py`.
- **2026-06-15** — **§13 local staging: works, but ~no speedup (honest correction).** Staged v1.0 RGB+labels (15 GB) to `/mnt/outputs/v1.0/data_local`; epoch time **~122 s local vs ~124 s GCS** under 8-way concurrency → training is **compute-bound, not I/O-bound**; the 0%-util troughs were validation/checkpoint phases, not per-step GCS fetches. Staging still earns its place (removes the network-stall failure mode — cf. the 77-min GDAL/GCS hang — and any cross-region egress; the active VM is **us-central1-a**, infra-doc drift fixed). Real saturation win remains **never-idle between-wave scheduling**, not within-run I/O.
- **2026-06-15** — **2024-label validation of the baseline overlay.** The Banks Island tiny-area AOI (2025 imagery) contains **11 v1.0 *2024* positive tiles** (139,995 RTS label px). Overlaid them on the merged probability (`scripts/annotate_overlay_labels.py` → `validation_overlay_2024labels.png`): **mean model prob 0.578 inside 2024 RTS labels vs 0.071 outside (8×)** — strong qualitative confirmation the locked baseline fires on real RTS. Also rebuilt the project report (`scripts/merge_mlflow_stores.py` → combined store → `build_report.py` → `/mnt/outputs/v1.0/report.html`).
