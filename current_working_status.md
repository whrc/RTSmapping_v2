...
- **Next two steps**:
  ...
  2. 🔄 **Migrating to `a100-8x-train` (8× A100-80GB, us-central1-a)** — landed 2026-06-12 03:22 UTC after 336 spin-retry attempts; see `computing/migrate_vm.md` (the handover doc + checklist). Backlog to run there as per-GPU parallel queues: rerun `phase3_boundary_ignore_w2/w3`, `phase3_wd_5e2`, `phase3_aug_strong` (all crashed ~60 s in on transient GCS 404s at 04:10 UTC; summaries show best_epoch=-1) + new `phase3_loss_compound_2to1_seed44` tiebreak. Then: data-scaling slope (§5.3) + Phase-5 gate eval (§8.1) + §6.4 Phase-3 lock. H100 spin-retry to be stopped; `ml-training-vm` to be stopped after validation.
