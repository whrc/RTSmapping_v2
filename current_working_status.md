...
- 2026-06-12 (evening) — **v2.1 pivot confirmed + interim plan while negatives generate.** User confirmed the bucket rewrite is the intentional **v2.1 drop**: 1,757 quality-filtered positives (batch1: 1169 / batch2: 261 / batch3: 327) are the canonical positive set; negatives regenerating upstream (~12 h ETA); EXTRA definition still settling (inference EXTRA prepared after it locks). `computing/migrate_vm.md` blocker section rewritten as the v2.0→v2.1 transition; v2.0 negatives unrecoverable (frozen inventory kept at `/mnt/outputs/phase0c_frozen_metadata.csv` for the data team). Git tidy-up: **PR #15 opened (`phase1-prep` → `main`)**, `gh` CLI installed + authed; branches: `v21-rebaseline` (future re-baseline training), `docs-eval-interim` (docs + evaluation), `inference-pipeline` (Phase-2 inference build, started now against `gs://pdg-planet-data` 2025-Q3 quads — 4096² uint8 RGBA, EPSG:3857, zoom-15). Noted: §5.3 slope fit + §5.4 gap analysis already complete in `docs/phase2_data_scaling.md` (ratio 4.4 = severely under-scaled; gap 0.43 = over-parameterized; Phase 5 lean-skip, formal skip doc deferred to Phase 3/4 lock) — all v2.0-based, to re-confirm on v2.1. Staleness audit of locked v2.0 decisions: `docs/v21_staleness_audit.md`.
