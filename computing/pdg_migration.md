...
| # | Check | Result |
|---|---|---|
| 1 | Parity per prefix — object count and total bytes, source vs destination. Acquisition prefixes must use the **frozen** Phase-C measurement, not the provisional Phase-B one | **6 of 7 legs PASS** 2026-08-28, every object compared by name, size and checksum via `gcs_parity.py` — not sampled. `planet` **5,000,891 objects / 39.46 TB identical** and `s2` **14,780 / 20.72 TB identical**, both measured against genuinely frozen sources (export abandoned, order loop stopped, delivery tail drained) after `planet-final` moved 61,791 objects / 428 GB and `s2-final` moved exactly the 5 objects the pre-freeze diff predicted. `experiments` 253,035 / 272.81 GB, `ee_mirror` 4,061 / 13.99 GB, `ee_staging` 3, `interannual` 4 — all exact. `review` separately diffed for the cutover: nothing missing, nothing differing (row 7). `inference` (42.3 M objects, ~85 M comparisons) still walking. |
...
| 6 | Live acquisition unbroken — `check_status.py` advancing into the new bucket, `ord/min` back at 38–39 | **PENDING — deliberately paused, not broken.** The 2019 run was stopped 2026-08-28 at **215,443 / 308,686 orders (69.8 %), 0 failed** because it was ordering into `pdg-planet-data`, which dies on the 31st. `rts-ops` is now provisioned to take it over (venv, work dir, webhook, alert cron — none of which existed before today) and `check_status.py` reads the migrated state there. This row closes when Heidi restarts on `rts-ops` **after** the delivery tail drains and `planet-final` completes, and `ord/min` returns to 38–39 against `rts-arctic-usw1`. Ordering is the constraint, not the restart — see cutover §3. |
...
| 8 | Public EE map renders from the new assets and the new usc1 mirror | **PARTIAL** 2026-08-28 — the data side is verified: all **six** assets the public apps actually reference (`south_rts`, `south_rts_t65`, `south_rts_candidates`, `south_rts_centroids`, `south_mask`, `south_density_10km`) exist under `projects/abruptthawmapping/assets/` and are `all_users_can_read`, matching the PDG originals exactly — a private asset would render the public app blank for everyone but us, which is the failure this row exists to catch. The `ee_mirror` leg backing `loadGeoTIFF` is parity-MATCH (4,061 objects / 13.99 GB, byte-identical). **Still needs a human:** publishing is a Code Editor UI action, so the repointed `ee_south_app.js` / `ee_south_viewer.js` must be pasted and Updated by hand before the live app reads the new project. |
