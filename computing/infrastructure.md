...
| **Experiment artifacts** | MLflow runs + artifacts, in-container `requirements_frozen`, preview visualizations, phase reports (e.g. `phase0_report.html`) | `/mnt/outputs/mlflow` (per-VM, local file store) | shared concurrency-safe backend + artifact bucket for the multi-node phase (see §7) | Per-VM store does not aggregate across parallel nodes. |
