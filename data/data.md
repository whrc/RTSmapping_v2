...
**splits.yaml format** — lists Arctic region names per split (not individual tile IDs):
```yaml
train:
  - yakutia
  - alaska_northslope
...
Note: TrainClass values are `Positive` or `Negative` only. Hard negatives (from Lingcao Huang's model false positives) are stored as `Negative` — no separate class needed.
UIDs are RTS UIDs contained within the tile (used for tracking individual RTS); empty for Negative tiles.
RegionName is Arctic subregion defined by ecology/permafrost extent (boundaries provided by Heidi Rodenhizer).
split.yaml (e.g.):
```
train:
  - yakutia
...
Additional channels may be added in future (e.g. slope, aspect from ArcticDEM).

**Channel selection at training time**: Specified in YAML config by name:
```yaml
extra_channels: [ndvi, nir, re, sr]   # subset or all channels
```
...
2. Resampled to match PlanetScope resolution (~3 m) using **bilinear interpolation** for all channels
...
- Clipping percentile decision is pending collaborator review; default to 1–99% until confirmed
...
**Region definitions**: Provided by Heidi Rodenhizer (boundaries complete). Storage format TBD.
