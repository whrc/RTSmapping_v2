...
train_hex = train_points |>
  ...
  mutate(
    Buffer = (sqrt((100000 * 2) / (3 * sqrt(3))) * sqrt(3) * 1000) /
      2 *
      (1 - sqrt(Count / max(Count))), # hexagon short side length * percentile of total count
...
train_hexplot = train_hexplot +
  inset_element(legend, left = 0.7, bottom = 0, right = 0.995, top = 0.21)
```

```{r}
ggsave(
  "./plots/training_data_vis.png",
  train_hexplot,
  height = 8.2,
  width = 6.5
)
